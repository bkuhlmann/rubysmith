  ...
  describe "#call" do
    ...
    it "answers array with attributes path appended to default path" do
      expect(transformer.call(attributes)).to be_success(
        template_roots: [Bundler.root.join("lib/rubysmith/templates"), Pathname.pwd]
      )
    end
    ...
    it "answers array without defaults" do
      ...
      expect(transformer.call(attributes)).to be_success(template_roots: [Pathname.pwd])
    end

    it "answers empty array without defaults or custom attributes" do
      transformer = described_class.new default: nil
      expect(transformer.call({})).to be_success({template_roots: []})
    end
